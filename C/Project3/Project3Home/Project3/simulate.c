	...
	while(completeQ.Qcount != 46){
		...
		if(readyQ.Qcount > 0){
			...
			if(cpujob.dispatchedOnce == FALSE){
				...
				//printf("cpujob acquire is: %lf, cpujob arrival is: %lf\n", cpujob.acquireTime, cpujob.arrivalTime);
				//printf("cpujob num is: %d, cpujob.waitTime is: %lf\n", cpujob.number, cpujob.waitTime);
				//printf("totalWait is: %lf\n\n", totalWait);
		...
		if(cpuIdle == FALSE){
			...
			if(cpujob.partialQ == 0 && cpujob.serviceQs == 0){
				...
				//printf("cpujob num is: %d, cpujob complete is: %lf\n", cpujob.number, cpujob.completeTime);
				//printf("inside if\n");
